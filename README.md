...
Conflicting on the same `Pingable` instance is much worse however. While 2 `Pinger`s still get ~3 million rps, 4 `Pinger`s go down to ~1 million rps, then ~750k rps for 128 `Pinger`s, ~670k rps for 256, ~630k for 512, ~320k for 1024, ~110k for 2048, etc. I'm not sure what's going on, but it doesn't look good. Either there's some kind of a thundering herd problem (there's only 1 core consumed at the time, so it doesn't really look like it however), or something is very suboptimal here.
